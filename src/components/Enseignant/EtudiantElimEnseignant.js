...
const EtudiantElimEnseignant = (props) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/elimine/etudiantsElimines/${classeId}/${matiereId}`,
        {
          headers: {
  ...
  const filename = `${data?.classe}_${data?.matiere}_elimines.csv`;
      ...
      <div className="table-responsive text-nowrap">
        ...
        >
          ...
          <label
            ...
            style={{ color: "#4CAF50" }}
          >
            Nombre d'Éliminés : {data?.etudiants ? data.etudiants.length : 0}
          </label>
        </div>
        <br></br>
        <br></br>
...
export default EtudiantElimEnseignant;
